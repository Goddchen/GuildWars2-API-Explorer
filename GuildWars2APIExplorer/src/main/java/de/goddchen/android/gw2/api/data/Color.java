...
//            2) Apply brightness and contrast
            for (int i = 0; i < 3; i++) {
                baseRgb[i] = (int) (((baseRgb[i] + brightness) - 128) * contrast + 128);
            }

//            3) Convert color to HSL space
            float[] hsl = RGBtoHSL(baseRgb[0], baseRgb[1], baseRgb[2], null);

//            4) Apply HSL shift
            hsl[0] = (hsl[0] * 360 + hue) / 360;
            hsl[1] = (float) (hsl[1] * saturation);
            hsl[2] = (float) (hsl[2] * lightness);
